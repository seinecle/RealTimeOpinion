...
public class Bean implements Serializable {
    ...
    private Map<String, Float> mapIPToGrade;
    private Map<String, Long> mapIPToTime;
    private Float averageGrade;
    private Integer groupSize;

    public Bean() {
        createMeterGaugeModel(50f);
    }
    ...
    public void setSessionCode(String sessionCode) {
        ...
        if (!sharedBean.getMapSessionCodesToDay().containsKey(sessionCode)) {
            sharedBean.getMapSessionCodesToDay().put(sessionCode, LocalDate.now());
    ...
    public void confirmGrade() {
        try {
            ...
            if (grade != null && ipAddress != null) {
                ...
                mapIPToGrade.put(ipAddress, grade * 50f);
            ...
            for (Entry<String, Float> entry : mapIPToGrade.entrySet()) {
                sumGrades = (float)sumGrades + entry.getValue();
            }
            averageGrade = (float) sumGrades / numberOfGrades;

            meterGaugeChartModel = createMeterGaugeModel(averageGrade);
            meterGaugeChartModel.setSeriesColors("ff0000, ff8300, ffe500, a1ff00, 00ff04");
    ...
    private MeterGaugeChartModel createMeterGaugeModel(float averageGrade) {
    ...
    private void findAndDeleteOldSessionCodes() {
        try {
            ...
            while (iterator.hasNext()) {
                entry = iterator.next();
                if (entry.getValue().isBefore(LocalDate.now().minusDays(1))) {
